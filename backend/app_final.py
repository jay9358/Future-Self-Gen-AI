# app.py - Fixed WebSocket event handlers and session validation
import os
import sys
import json
import uuid
...
# Import the enhanced resume analyzer
...
def validate_session(session_id):
    ...
    if not session_id:
        logger.warning("No session_id provided for validation")
        return False
...
def handle_join_session(data):
    ...
    client_id = request.sid
    session_id = data.get('session_id') or data.get('session_id')
    ...
    # For now, create session if it doesn't exist (for development)
    session_data = session_service.get_session(session_id)
    if not session_data:
        ...
        session_service.create_session(session_id, session_data)
...
# Add remaining endpoints here (career-database, etc.)...
